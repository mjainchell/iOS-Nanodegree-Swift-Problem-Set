...
func checkForRelease(bugs: Int, music: Bool, levels: Int) {
    if bugs < 10 && music == true && levels > 5 {
        release()
    }else if bugs >= 10 || music == false || levels <= 5{
        if bugs >= 10 {
            print("The game: \(game), has \(bugs) bugs.")
        }
        if music == false {
            print("The game: \(game), needs music.")
        }
        if levels <= 5 {
            print("The game: \(game), only has \(levels) levels")
        }
    }
}
...
func checkTrainingStatus(name: String, bike: Bool, run: Bool) {
    ...
    if bike == true && run == true {
        ...
        print("\(name) cannot finish biking or running portion!")
    }
}
