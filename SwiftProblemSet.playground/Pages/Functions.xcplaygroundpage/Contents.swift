...
func median(num1: Int, num2: Int, num3: Int) -> Int {
    /* add function implementation here */
    let theNumbers = [num1,num2,num3].sorted()
    let theMedian = theNumbers[1]
    return theMedian
}
 ...
 */
func beginsWithVowel(_ theWord: String) -> Bool {
    var decision = false
    if theWord.characters.count == 0 {
      decision = false
    }else if theWord.characters.count != 0 {
        let firstLetter = theWord.lowercased().characters[theWord.startIndex]
 ...
 */
func funWithWords(_ theWord: String) -> String {
    var theOutputString: String
    if beginsWithVowel(theWord) {
        theOutputString = theWord.uppercased()
    }else {
        theOutputString = theWord.lowercased()
    }
